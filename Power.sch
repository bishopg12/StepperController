EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr USLedger 17000 11000
encoding utf-8
Sheet 9 13
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L ICs~-~Temperature~Sensors~-~Analog~and~Digital~Output:swr1000000001755 U9
U 1 1 5F29A77E
P 2050 1450
F 0 "U9" H 2075 1865 50  0000 C CNN
F 1 "swr1000000001755" H 2900 2100 50  0001 C CNN
F 2 "ICs:6-UFBGA" H 2050 1450 50  0001 C CNN
F 3 "" H 2050 1450 50  0001 C CNN
F 4 "TMP108AIYFFR" H 2075 1774 50  0000 C CNN "PartValue"
F 5 "0.625mm" H 2050 1450 50  0001 C CNN "Height"
	1    2050 1450
	1    0    0    -1  
$EndComp
Text HLabel 1600 1450 0    50   Input ~ 0
TMP_SCL
Text HLabel 1600 1600 0    50   BiDi ~ 0
TMP_SDA
Text HLabel 2550 1450 2    50   Input ~ 0
TMP_A0
Text HLabel 2550 1600 2    50   Output ~ 0
TMP_~ALERT
$Comp
L power:+3.3V #PWR0102
U 1 1 5F29AC87
P 3150 1200
F 0 "#PWR0102" H 3150 1050 50  0001 C CNN
F 1 "+3.3V" H 3165 1373 50  0000 C CNN
F 2 "" H 3150 1200 50  0001 C CNN
F 3 "" H 3150 1200 50  0001 C CNN
	1    3150 1200
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0101
U 1 1 5F29AE83
P 2550 1850
F 0 "#PWR0101" H 2550 1600 50  0001 C CNN
F 1 "GND" H 2555 1677 50  0000 C CNN
F 2 "" H 2550 1850 50  0001 C CNN
F 3 "" H 2550 1850 50  0001 C CNN
	1    2550 1850
	1    0    0    -1  
$EndComp
$Comp
L Capacitors:swr100000000824 C77
U 1 1 5F29B135
P 3150 1550
F 0 "C77" H 3265 1687 50  0000 L CNN
F 1 "swr100000000824" H 3330 1050 50  0001 C CNN
F 2 "Capacitors:C_0402_1005Metric" H 3150 2100 50  0001 C CNN
F 3 "" H 3150 1550 50  0001 C CNN
F 4 "0.1uF" H 3265 1596 50  0000 L CNN "PartValue"
F 5 "16V" H 3265 1505 50  0000 L CNN "Voltage"
F 6 "5%" H 3265 1414 50  0000 L CNN "Tolerance"
	1    3150 1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	2450 1450 2550 1450
Wire Wire Line
	2450 1600 2550 1600
Wire Wire Line
	2550 1850 2550 1750
Wire Wire Line
	2550 1750 2450 1750
Wire Wire Line
	1700 1450 1600 1450
Wire Wire Line
	1600 1600 1700 1600
$Comp
L power:GND #PWR0103
U 1 1 5F29BA23
P 3150 1800
F 0 "#PWR0103" H 3150 1550 50  0001 C CNN
F 1 "GND" H 3155 1627 50  0000 C CNN
F 2 "" H 3150 1800 50  0001 C CNN
F 3 "" H 3150 1800 50  0001 C CNN
	1    3150 1800
	1    0    0    -1  
$EndComp
Wire Wire Line
	3150 1800 3150 1700
Wire Wire Line
	3150 1400 3150 1300
Wire Wire Line
	3150 1300 2450 1300
Wire Wire Line
	3150 1200 3150 1300
Connection ~ 3150 1300
$Comp
L Connectors:swr1000000001726 J9
U 1 1 5F2B1BE0
P 14150 1700
F 0 "J9" H 14092 1335 50  0000 C CNN
F 1 "swr1000000001726" H 14150 2050 50  0001 C CNN
F 2 "Connectors:575-4" H 14150 2150 50  0001 C CNN
F 3 "" H 14150 1700 50  0001 C CNN
F 4 "575-4" H 14092 1426 50  0000 C CNN "PartValue"
F 5 "2.36mm" H 14150 1950 50  0001 C CNN "Height"
	1    14150 1700
	-1   0    0    1   
$EndComp
Text HLabel 15000 1600 2    50   Output ~ 0
X_AXIS_VS
$Comp
L Connectors:swr1000000001726 J10
U 1 1 5F2B2C8B
P 14150 2200
F 0 "J10" H 14092 1835 50  0000 C CNN
F 1 "swr1000000001726" H 14150 2550 50  0001 C CNN
F 2 "Connectors:575-4" H 14150 2650 50  0001 C CNN
F 3 "" H 14150 2200 50  0001 C CNN
F 4 "575-4" H 14092 1926 50  0000 C CNN "PartValue"
F 5 "2.36mm" H 14150 2450 50  0001 C CNN "Height"
	1    14150 2200
	-1   0    0    1   
$EndComp
$Comp
L Connectors:swr1000000001726 J11
U 1 1 5F2B30B4
P 14150 2700
F 0 "J11" H 14092 2335 50  0000 C CNN
F 1 "swr1000000001726" H 14150 3050 50  0001 C CNN
F 2 "Connectors:575-4" H 14150 3150 50  0001 C CNN
F 3 "" H 14150 2700 50  0001 C CNN
F 4 "575-4" H 14092 2426 50  0000 C CNN "PartValue"
F 5 "2.36mm" H 14150 2950 50  0001 C CNN "Height"
	1    14150 2700
	-1   0    0    1   
$EndComp
$Comp
L Connectors:swr1000000001726 J5
U 1 1 5F2B32DE
P 13150 1700
F 0 "J5" H 13092 1335 50  0000 C CNN
F 1 "swr1000000001726" H 13150 2050 50  0001 C CNN
F 2 "Connectors:575-4" H 13150 2150 50  0001 C CNN
F 3 "" H 13150 1700 50  0001 C CNN
F 4 "575-4" H 13092 1426 50  0000 C CNN "PartValue"
F 5 "2.36mm" H 13150 1950 50  0001 C CNN "Height"
	1    13150 1700
	-1   0    0    1   
$EndComp
$Comp
L Connectors:swr1000000001726 J7
U 1 1 5F2B3555
P 13150 2200
F 0 "J7" H 13092 1835 50  0000 C CNN
F 1 "swr1000000001726" H 13150 2550 50  0001 C CNN
F 2 "Connectors:575-4" H 13150 2650 50  0001 C CNN
F 3 "" H 13150 2200 50  0001 C CNN
F 4 "575-4" H 13092 1926 50  0000 C CNN "PartValue"
F 5 "2.36mm" H 13150 2450 50  0001 C CNN "Height"
	1    13150 2200
	-1   0    0    1   
$EndComp
$Comp
L Connectors:swr1000000001726 J8
U 1 1 5F2B3771
P 13150 2700
F 0 "J8" H 13092 2335 50  0000 C CNN
F 1 "swr1000000001726" H 13150 3050 50  0001 C CNN
F 2 "Connectors:575-4" H 13150 3150 50  0001 C CNN
F 3 "" H 13150 2700 50  0001 C CNN
F 4 "575-4" H 13092 2426 50  0000 C CNN "PartValue"
F 5 "2.36mm" H 13150 2950 50  0001 C CNN "Height"
	1    13150 2700
	-1   0    0    1   
$EndComp
Text HLabel 15000 2100 2    50   Output ~ 0
Y_AXIS_VS
Text HLabel 15000 2600 2    50   Output ~ 0
Z_AXIS_VS
$Comp
L power:GND #PWR0128
U 1 1 5F2B3A3B
P 13500 2700
F 0 "#PWR0128" H 13500 2450 50  0001 C CNN
F 1 "GND" H 13505 2527 50  0000 C CNN
F 2 "" H 13500 2700 50  0001 C CNN
F 3 "" H 13500 2700 50  0001 C CNN
	1    13500 2700
	1    0    0    -1  
$EndComp
Wire Wire Line
	13400 2600 13500 2600
Connection ~ 13500 2600
Wire Wire Line
	13500 2600 13500 2700
Wire Wire Line
	13400 2100 13500 2100
Wire Wire Line
	13500 2100 13500 2600
Wire Wire Line
	14400 1600 15000 1600
Wire Wire Line
	14400 2100 15000 2100
Wire Wire Line
	14400 2600 15000 2600
Text Label 14500 1600 0    50   ~ 0
X_AXIS_VS
Text Label 14500 2100 0    50   ~ 0
Y_AXIS_VS
Text Label 14500 2600 0    50   ~ 0
Z_AXIS_VS
Wire Wire Line
	13400 1600 13500 1600
Wire Wire Line
	13500 1600 13500 2100
Connection ~ 13500 2100
Text HLabel 2500 3650 0    50   Input ~ 0
+12V_AUX1
Wire Wire Line
	2500 3650 3000 3650
Text HLabel 2500 3950 0    50   Input ~ 0
DC_DC_X_AXIS_ENABLE
Wire Wire Line
	2500 3950 3000 3950
$Sheet
S 7000 3500 1000 1500
U 5F0912A5
F0 "DC_DC_Y_Axis_Driver" 50
F1 "DC_DC_Axis_Driver.sch" 50
F2 "+12V" I L 7000 3650 50 
F3 "ENABLE" I L 7000 3950 50 
F4 "VS" O R 8000 3650 50 
F5 "SYNC" I L 7000 4300 50 
F6 "SCL" I L 7000 4500 50 
F7 "SDA" B L 7000 4600 50 
F8 "~ALERT" O L 7000 4700 50 
F9 "A0" I L 7000 4800 50 
F10 "A1" I L 7000 4900 50 
$EndSheet
Text HLabel 4500 3650 2    50   Output ~ 0
X_AXIS_VS
Wire Wire Line
	4000 3650 4500 3650
Text HLabel 6500 3950 0    50   Input ~ 0
DC_DC_Y_AXIS_ENABLE
Wire Wire Line
	6500 3950 7000 3950
Text HLabel 6500 3650 0    50   Input ~ 0
+12V_AUX2
Wire Wire Line
	6500 3650 7000 3650
$Sheet
S 11000 3500 1000 1500
U 5F0A677D
F0 "DC_DC_Z_Axis_Driver" 50
F1 "DC_DC_Axis_Driver.sch" 50
F2 "+12V" I L 11000 3650 50 
F3 "ENABLE" I L 11000 3950 50 
F4 "VS" O R 12000 3650 50 
F5 "SYNC" I L 11000 4300 50 
F6 "SCL" I L 11000 4500 50 
F7 "SDA" B L 11000 4600 50 
F8 "~ALERT" O L 11000 4700 50 
F9 "A0" I L 11000 4800 50 
F10 "A1" I L 11000 4900 50 
$EndSheet
Text HLabel 10500 3650 0    50   Input ~ 0
+12V_AUX2
Text HLabel 10500 3950 0    50   Input ~ 0
DC_DC_Z_AXIS_ENABLE
Text HLabel 8500 3650 2    50   Output ~ 0
Y_AXIS_VS
Wire Wire Line
	10500 3650 11000 3650
Wire Wire Line
	10500 3950 11000 3950
Wire Wire Line
	8000 3650 8500 3650
Text HLabel 12500 3650 2    50   Output ~ 0
Z_AXIS_VS
Wire Wire Line
	12000 3650 12500 3650
Text HLabel 10500 4300 0    50   Input ~ 0
Z_AXIS_SYNC
Wire Wire Line
	10500 4300 11000 4300
Text HLabel 6500 4300 0    50   Input ~ 0
Y_AXIS_SYNC
Text HLabel 2500 4300 0    50   Input ~ 0
X_AXIS_SYNC
Wire Wire Line
	2500 4300 3000 4300
Wire Wire Line
	6500 4300 7000 4300
$Sheet
S 3000 3500 1000 1500
U 5F081803
F0 "DC_DC_X_Axis_Driver" 50
F1 "DC_DC_Axis_Driver.sch" 50
F2 "+12V" I L 3000 3650 50 
F3 "ENABLE" I L 3000 3950 50 
F4 "VS" O R 4000 3650 50 
F5 "SYNC" I L 3000 4300 50 
F6 "SCL" I L 3000 4500 50 
F7 "SDA" B L 3000 4600 50 
F8 "~ALERT" O L 3000 4700 50 
F9 "A0" I L 3000 4800 50 
F10 "A1" I L 3000 4900 50 
$EndSheet
Wire Wire Line
	3000 4500 2500 4500
Wire Wire Line
	3000 4600 2500 4600
Wire Wire Line
	3000 4700 2500 4700
Wire Wire Line
	7000 4500 6500 4500
Wire Wire Line
	7000 4600 6500 4600
Wire Wire Line
	7000 4700 6500 4700
Wire Wire Line
	11000 4500 10500 4500
Wire Wire Line
	11000 4600 10500 4600
Wire Wire Line
	11000 4700 10500 4700
Text HLabel 10500 4500 0    50   Input ~ 0
POWER_SCL
Text HLabel 10500 4600 0    50   BiDi ~ 0
POWER_SDA
Text HLabel 10500 4700 0    50   Output ~ 0
POWER_~ALERT
Text HLabel 6500 4500 0    50   Input ~ 0
POWER_SCL
Text HLabel 6500 4600 0    50   BiDi ~ 0
POWER_SDA
Text HLabel 6500 4700 0    50   Output ~ 0
POWER_~ALERT
Text HLabel 2500 4500 0    50   Input ~ 0
POWER_SCL
Text HLabel 2500 4600 0    50   BiDi ~ 0
POWER_SDA
Text HLabel 2500 4700 0    50   Output ~ 0
POWER_~ALERT
$Comp
L power:GND #PWR0160
U 1 1 5F2A4643
P 2900 5000
F 0 "#PWR0160" H 2900 4750 50  0001 C CNN
F 1 "GND" H 2905 4827 50  0000 C CNN
F 2 "" H 2900 5000 50  0001 C CNN
F 3 "" H 2900 5000 50  0001 C CNN
	1    2900 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	3000 4800 2900 4800
Wire Wire Line
	2900 4800 2900 4900
Wire Wire Line
	3000 4900 2900 4900
Connection ~ 2900 4900
Wire Wire Line
	2900 4900 2900 5000
Text Notes 2300 4900 0    50   ~ 0
Address: 0x40
$Comp
L power:+3.3V #PWR0161
U 1 1 5F2A6B3D
P 6800 4800
F 0 "#PWR0161" H 6800 4650 50  0001 C CNN
F 1 "+3.3V" V 6815 4928 50  0000 L CNN
F 2 "" H 6800 4800 50  0001 C CNN
F 3 "" H 6800 4800 50  0001 C CNN
	1    6800 4800
	0    -1   -1   0   
$EndComp
Wire Wire Line
	6800 4800 6900 4800
Wire Wire Line
	7000 4900 6900 4900
Wire Wire Line
	6900 4900 6900 4800
Connection ~ 6900 4800
Wire Wire Line
	6900 4800 7000 4800
Text Notes 6200 4950 0    50   ~ 0
Address: 0x45
$Comp
L power:+3.3V #PWR0162
U 1 1 5F2A9C0A
P 10900 4800
F 0 "#PWR0162" H 10900 4650 50  0001 C CNN
F 1 "+3.3V" V 10915 4928 50  0000 L CNN
F 2 "" H 10900 4800 50  0001 C CNN
F 3 "" H 10900 4800 50  0001 C CNN
	1    10900 4800
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR0163
U 1 1 5F2A9F6B
P 10900 5000
F 0 "#PWR0163" H 10900 4750 50  0001 C CNN
F 1 "GND" H 10905 4827 50  0000 C CNN
F 2 "" H 10900 5000 50  0001 C CNN
F 3 "" H 10900 5000 50  0001 C CNN
	1    10900 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	11000 4800 10900 4800
Wire Wire Line
	11000 4900 10900 4900
Wire Wire Line
	10900 4900 10900 5000
Text Notes 10250 5000 0    50   ~ 0
Address: 0x41
$EndSCHEMATC
